#making a music player with python
...
class MusicPlayer:
    def __init__(self,root):
        ...
        TrackStstus=Label(TrackFrame,textvariable=self.ststus,font=("times new roman",24,"bold"),bg="grey",fg="gold").grid(row=0,column=1,padx=10,pady=5)
        ...
        os.chdir("C:\Users\91636\OneDrive\Desktop\Chotu codes")
        songtracks=os.listdir()
        for track in songtracks:
            self.playlist.insert(END,track)
        def playsong(self):
            self.track.set(self.playlist.get(ACTIVE))
            self.ststus.set("-Playing")
            pygame.mixer.music.load(self.playlist.get(ACTIVE))
            pygame.mixer.music.play()
        def stopsong(self):
            self.ststus.set("-Stopped")
            pygame.mixer.music.stop()
        def pausesong(self):
            self.ststus.set("-Paused")
